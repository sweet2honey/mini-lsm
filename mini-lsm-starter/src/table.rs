...
impl BlockMeta {
    ...
    pub fn encode_block_meta(block_meta: &[BlockMeta], buf: &mut Vec<u8>) {
        let append_size = block_meta
            .iter()
            .map(|meta| size_of::<u16>() + meta.first_key.len() + meta.last_key.len())
            .sum::<usize>();
...
impl SsTable {
    ...
    pub fn open(id: usize, block_cache: Option<Arc<BlockCache>>, file: FileObject) -> Result<Self> {
        ...
        // last 4 bytes is `meta_offset`
        let meta_offset = file.read(file_size - 4, 4)?.as_slice().get_u32() as u64;
        let meta_bytes = file.read(meta_offset, file_size - 4 - meta_offset)?;
    ...
    pub fn read_block(&self, block_idx: usize) -> Result<Arc<Block>> {
        ...
        let block_end = self
            .block_meta
            .get(block_idx + 1)
            .map_or(self.block_meta_offset, |next_meta| next_meta.offset);

        // Read from file
        let block_data = self
            .file
            .read(block_start as u64, (block_end - block_start) as u64)?;
    ...
    pub fn find_block_idx(&self, key: KeySlice) -> usize {
        self.block_meta
            // `partition_point` returns the index of the block of which `first_key > key`
            .partition_point(|meta| meta.first_key.as_key_slice() <= key)
            // then we go back one block
            .saturating_sub(1)
    }
