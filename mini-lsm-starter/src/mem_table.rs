...
impl MemTable {
    ...
    /// Get a value by key. Returns the raw stored value verbatim, including the
    /// empty value used to mark a tombstone; the caller decides what it means.
    ...
    pub fn scan(&self, lower: Bound<&[u8]>, upper: Bound<&[u8]>) -> MemTableIterator {
        ...
        .build();
        // Position the cursor at the first entry inside [lower, upper]. An empty key
        // in `item` is the end-of-range sentinel; see `is_valid`.
        iterator.with_mut(|fields| {
